...
import { USER_TYPES, ResponseStatus, UserStatus, USER_STATUS, SMS_USER_LEVEL } from '../../common/QSEnums';
import { VMUnit } from '../../models/smModels/vmUnit';
import { UnitService } from '../../services/unitService';

@Component({
  selector: 'app-smsuser-list',
  templateUrl: './smsuser-list.component.html',
  styleUrls: ['./smsuser-list.component.css'],
...
export class SMSUserListComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.lstUserStatus = USER_STATUS;
  ...
  public getAllUser() {
    ...
    this.userService.getUsersByType(USER_TYPES[1].id).subscribe((response) => {
      if (response.StatusCode == ResponseStatus.success) {
  ...
  public saveUser() {
    ...
    if (!this.chackPassword()) {
      this.objUser.UserType = USER_TYPES[1].id;

      this.userService.updateUser(this.objUser).subscribe((response) => {
        if (response.StatusCode == ResponseStatus.success) {
